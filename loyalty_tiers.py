...
def main():
    ...
    # GraphQL - no filters, just get all orders
    query = f"""
    {{
      customer(id: "{gql_id}") {{
        numberOfOrders
    ...
    for edge in orders:
        o = edge["node"]
        print(f"  {o['name']} created:{o['createdAt'][:10]} processed:{o['processedAt'][:10]} status:{o['displayFinancialStatus']} ${o['totalPriceSet']['shopMoney']['amount']}")
